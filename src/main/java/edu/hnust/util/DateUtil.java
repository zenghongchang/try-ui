...
public class DateUtil
{
    ...
    public static String calDiffTime(String oneTime,String twoTime){
    	try  {   
	   		DateFormat df = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");  
	   	    Date d1 = df.parse(oneTime);  
	   	    Date d2 = df.parse(twoTime);  
	   	    long diff = d1.getTime() - d2.getTime();//得到的差值是微秒级别  
	   	    long day=diff/(24*60*60*1000);
		    long hour=(diff/(60*60*1000)-day*24);
		    long min=((diff/(60*1000))-day*24*60-hour*60); 
		    long s=(diff/1000-day*24*60*60-hour*60*60-min*60);
		    String onlineTime=day+"天"+hour+"小时"+min+"分"+s+"秒";
	   	    return onlineTime;
	   	}catch (Exception e) {
	   		e.printStackTrace();
	   	}  
    ...
    public static long calIntervalTime(String oneTime,String twoTime){
    	try  {   
	   		DateFormat df = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");  
	   	    Date d1 = df.parse(oneTime);  
	   	    Date d2 = df.parse(twoTime);  
