...
属性说明：
 ...
 使用方式：
  <#if recordCount??>
    ...
    <@q.pager pager=pager  toURL="testpager.do"/>
  </#if>
 -->
<#macro page pager toURL >
<div class="view_log left_float view_log_container2">
    <input type="hidden" id ="requestUrl" name="requestUrl" value="${toURL}"/>
    <input type="hidden" id ="currentPage" name="currentPage" value="${pager.currentPage!"1"}"/>
    <input type="hidden" id ="totalPages" name="totalPages" value="${pager.totalPages!"0"}"/>
  <@spring.message "fbt.page.di"/><div class="first_page">
			<div class="left_page left_float"> &lt; </div>
			<div class="number_page left_float"><#if pager??><input  type="text" value="1" class="input_shuru input_none" id="page_number" value="${pager.currentPage}"><#else>1</#if></div>
			<div class="right_page left_float"> &gt; </div>
	</div>
	<@spring.message "fbt.page.ji.ye"/><span id="span_totalPages"><#if pager??>${pager.totalPages}<#else>0</#if></span><@spring.message "fbt.page.ye"/>
	<span class="vertical_line">|</span> 
	<span> <@spring.message "fbt.page.look"/> </span>
	<#if pager??>
	<select id="pager-limit" name="pageLimit" class="any_page">
		<#list pager.selects as select>
		<option <#if pager.pageLimit==select> selected="selected" </#if> value="${select}">
			${select}
		</option>
		</#list>
	</select>
	...
	</#if> <@spring.message "fbt.page.every.page"/>
	<span class="vertical_line">|</span>  <@spring.message "fbt.page.gong"/> <strong id="totalRecords"><#if pager??>${pager.totalRecords}<#else>0</#if></strong> <@spring.message "fbt.page.tiao.j.lu"/>
</div>
</#macro>
